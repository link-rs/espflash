...
/// Extended reset delay (100ms) used in reset sequences.
const EXTENDED_RESET_MS: u64 = 100;
    ...
    pub async fn reset<P: SerialInterface>(&self, serial_port: &mut P) -> Result<(), Error> {
        match self {
            ResetStrategy::Classic { delay_ms } => {
                ...
                serial_port.delay_ms(EXTENDED_RESET_MS as u32).await;
...
pub async fn reset_after_flash<P: SerialInterface>(
    serial: &mut P,
    pid: u16,
) -> Result<(), Error> {
...
pub async fn hard_reset<P: SerialInterface>(
    serial_port: &mut P,
    pid: u16,
) -> Result<(), Error> {
...
pub async fn soft_reset<P: SerialInterface>(
    ...
    if !is_stub {
        if stay_in_bootloader {
            ...
            connection
                .serial
                .set_timeout(CommandType::FlashBegin.timeout())?;
            let size: u32 = 0;
            let offset: u32 = 0;
            ...
            connection
                .serial
                .set_timeout(CommandType::FlashEnd.timeout())?;
            let result = connection
        ...
        connection
            .serial
            .set_timeout(CommandType::RunUserCode.timeout())?;
