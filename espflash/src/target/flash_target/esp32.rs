...
impl Esp32Target {
    ...
    pub async fn begin<P: SerialInterface>(
        &mut self,
        ...
        connection
            .serial
            .set_timeout(CommandType::SpiAttach.timeout())?;
    ...
    pub async fn write_segment<P: SerialInterface>(
        ...
        connection
            .serial
            .set_timeout(CommandType::FlashDeflBegin.timeout_for_size(erase_size))?;
        let result = connection
        ...
        for (i, block) in chunks.enumerate() {
            ...
            let old_timeout = connection.serial.timeout();
            connection.serial.set_timeout(
                CommandType::FlashDeflData.timeout_for_size(estimated_size as u32),
            )?;
            let result = connection
    ...
    pub async fn finish<P: SerialInterface>(
        &mut self,
        connection: &mut Connection<P>,
        reboot: bool,
    ) -> Result<(), Error> {
        if self.need_deflate_end {
            ...
            connection
                .serial
                .set_timeout(CommandType::FlashDeflEnd.timeout())?;
            let result = connection
